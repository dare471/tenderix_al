...
;(function(window){
	...
	window.EditEventPopupController.prototype = {
		...
		InitDestinationControls: function()
		{
			...
			BX.addCustomEvent('OnSetTab', BX.proxy(this.OnPlannerTapShow, this));
			...
			{
				_this.pFromDate.value = _this.oEC.FormatDate(params.dateFrom);
				_this.pFromTime.value = _this.oEC.FormatTime(params.dateFrom);
				_this.pToDate.value = _this.oEC.FormatDate(params.dateTo);
				_this.pToTime.value = _this.oEC.FormatTime(params.dateTo);

				_this.pFullDay.checked = false;
				_this.FullDay(false, true);
			});
		...
		DestroyDestinationControls: function()
		{
			...
			BX.removeCustomEvent('OnSetTab', BX.proxy(this.OnPlannerTapShow, this));
			BX.onCustomEvent('OnCalendarPlannerDoUninstall', [{plannerId: this.plannerId}]);
		},

		OnPlannerTapShow: function(tab, show)
		{
			if (show && tab.tab == BX(this.id + 'ed-tab-2'))
			{
				if (this.plannerLoadedTimezone !== this.pFromTz.value)
