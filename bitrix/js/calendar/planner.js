...
CalendarPlanner.prototype =
{
	...
	BuildSelector: function(params)
	{
		...
		//var selectorProposeIcon = selector.appendChild(BX.create("DIV", {props: {className: 'calendar-planner-warning'}, html: '<span class="calendar-planner-warning-item"></span>'}));
		//BX.bind(selectorProposeIcon, 'click', BX.proxy(this.ShowSelectorWarningPopup, this));
		//this.selectorProposeIcon.setAttribute('data-bx-planner-meta', 'selector-warning');

		return {
			selector: selector,
			selectorTitle: selectorTitle
			//selectorProposeIcon: selectorProposeIcon
		};
	},
	...
	//ScrollTimelineHandler: function()
	//{
	//	if (this.shown && !this.readonly)
	//	{
	//		var
	...
	ShowSelectorWarningPopup: function()
	{
		var
			_this = this,
			dialogCont = BX.create('DIV', {props: {className: 'calendar-planner-selector-warning-popup'}}),
			proposeLink = dialogCont.appendChild(BX.create('SPAN', {props: {className: 'calendar-planner-propose-link'}, html: '<span class="calendar-planner-propose-link-text">' + BX.message('EC_PL_PROPOSE') + '</span><span class="calendar-planner-popup-arrow-right-item"></span>'}));

		BX.bind(proposeLink, 'click', BX.proxy(this.ProposeTime, this));

		// Create and show settings popup
		this.selectorWarningPopup = BX.PopupWindowManager.create(this.id + "-warning-popup", this.selectorProposeIcon,
		{
			autoHide: true,
			closeByEsc: true,
			offsetTop: -14,
			...
			angle: {postion: 'bottom'}
		});
		this.selectorWarningPopup.show(true);

		function destroyPopup()
		{
			if(_this.selectorWarningPopup && _this.selectorWarningPopup.destroy)
			{
				BX.removeCustomEvent(_this.selectorWarningPopup, 'onPopupClose', destroyPopup);
				_this.selectorWarningPopup.destroy();
				_this.selectorWarningPopup = null;
			}
		}
		BX.addCustomEvent(_this.selectorWarningPopup, 'onPopupClose', destroyPopup);
	},

	HideSelectorWarningPopup: function()
	{
		if (this.selectorWarningPopup)
			this.selectorWarningPopup.close();
	},
