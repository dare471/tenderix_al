...
else
{
	...
	else
	{
		...
		else
		{
			...
			{
				case 'KZT':
					$url = 'http://www.nationalbank.kz/rss/get_rates.cfm?fdate='.$DB->FormatDate($date, CLang::GetDateFormat('SHORT', LANGUAGE_ID), 'D.M.Y');
					break;
				case 'UAH':
			...
			switch ($baseCurrency)
			{
				case 'KZT':
					if (is_array($data) && count($data["rates"]["#"]["item"])>0)
					{
						...
						{
							if ($currencyRate["#"]["title"][0]["#"] == $currency)
							{
								$result['STATUS'] = 'OK';
								$result['RATE_CNT'] = (int)$currencyRate["#"]["quant"][0]["#"];
								$result['RATE'] = (float)str_replace(",", ".", $currencyRate["#"]["description"][0]["#"]);
								break;
							}
						}
