...
if ((strlen($save) > 0 || strlen($apply) > 0) && $REQUEST_METHOD == "POST" && $TENDERIXRIGHT == "W" && check_bitrix_sessid()) {
    ...
    $res = 0;
    if ($ID > 0)
        $res = CTenderixSection::Update($ID, $arFields);
    else
        $res = CTenderixSection::Add($arFields);
...
if ($ID > 0) {
    $db_section = CTenderixSection::GetList($by, $order, array("ID" => $ID), $is_filtered);
...
$arrCatalog = CTenderixSection::BuildTree($arCat, 0, 0);
...
if ($ID > 0 && $TENDERIXRIGHT == "W") {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "/bitrix/admin/tenderix_section_edit.php?lang=" . LANG . "&" . GetFilterParams(array("CATALOG_ID"), false),
        "ICON" => "btn_new",
    );
    ...
    if ($ID != '1') {
        $aMenu[] = array(
            ...
            "LINK" => "javascript:if(confirm('" . GetMessage("PW_TD_DELETE_SECTION_CONFIRM") . "')) window.location='/bitrix/admin/tenderix_section.php?action=delete&ID[]=S" . $ID . "&lang=" . LANG . $sUrl . "&" . bitrix_sessid_get() . "#tb';",
            "ICON" => "btn_delete",
        );
    }
}
...
if ($message)
    echo $message->Show();
?>
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>" name="section_edit">
...
<?
$tabControl->ShowWarnings("section_edit", $message);
?>
