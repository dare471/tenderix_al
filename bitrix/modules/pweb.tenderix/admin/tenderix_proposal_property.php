...
$sTableID = "tbl_tenderix_proposal_property";
...
if ($lAdmin->EditAction() && $TENDERIXRIGHT >= "W" && check_bitrix_sessid()) {
    ...
    foreach ($FIELDS as $ID => $arFields) {
        ...
        if (!$DB->Update("b_tx_proposal_property", $arFieldsStore, "WHERE ID='$ID'", $err_mess . __LINE__)) {
    ...
    if ($bupdate)
        $CACHE_MANAGER->CleanDir("b_tx_proposal_property");
}
...
if (($arID = $lAdmin->GroupAction()) && $TENDERIXRIGHT == "W" && check_bitrix_sessid()) {
    if ($_REQUEST['action_target'] == 'selected') {
        $arID = Array();
        $rsData = CTenderixProposalProperty::GetList($by, $order, $arFilter);
    ...
    foreach ($arID as $ID) {
        ...
        switch ($_REQUEST['action']) {
            case "delete":
                @set_time_limit(0);
                $DB->StartTransaction();
                if (!CTenderixProposalProperty::Delete($ID)) {
                    $DB->Rollback();
            ...
            case "deactivate":
                ...
                if (!$DB->Update("b_tx_proposal_property", $arFields, "WHERE ID='$ID'", $err_mess . __LINE__))
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if ($TENDERIXRIGHT == "W") {
        ...
        $row->AddViewField("TITLE", '<a href="tenderix_proposal_property_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $f_ID . '" title="' . GetMessage("PW_TD_EDIT_TITLE") . '">' . $f_TITLE . '</a>');
    } else {
    ...
    $arActions = Array();
    $arActions[] = array("DEFAULT" => "Y", "ICON" => "edit", "TEXT" => GetMessage("MAIN_ADMIN_MENU_EDIT"), "ACTION" => $lAdmin->ActionRedirect("tenderix_proposal_property_edit.php?ID=" . $f_ID));
    $arActions[] = array("SEPARATOR" => true);
    $arActions[] = array("ICON" => "delete", "TEXT" => GetMessage("MAIN_ADMIN_MENU_DELETE"), "ACTION" => "if(confirm('" . GetMessage("PW_TD_CONFIRM_DEL_ENT") . "')) window.location='tenderix_proposal_property.php?lang=" . LANGUAGE_ID . "&action=delete&ID=$f_ID&" . bitrix_sessid_get() . "'");
...
if ($TENDERIXRIGHT == "W") {
    $aMenu[] = array(
        ...
        "LINK" => "tenderix_proposal_property_edit.php?lang=" . LANG,
        "ICON" => "btn_new"
    );
