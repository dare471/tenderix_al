...
$sTableID = "tbl_tenderix_spr_details";
...
if ($lAdmin->EditAction() && $TENDERIXRIGHT >= "W" && check_bitrix_sessid()) {
    ...
    foreach ($FIELDS as $ID => $arFields) {
        ...
        if (!$DB->Update("b_tx_spr_details", $arFieldsStore, "WHERE ID='$ID'", $err_mess . __LINE__)) {
    ...
    if ($bupdate)
        $CACHE_MANAGER->CleanDir("b_tx_spr_details");
}
...
if (($arID = $lAdmin->GroupAction()) && $TENDERIXRIGHT == "W" && check_bitrix_sessid()) {
    if ($_REQUEST['action_target'] == 'selected') {
        $arID = Array();
        $rsData = CTenderixSpr::GetList($by, $order, $arFilter, $is_filtered);
        ...
        switch ($_REQUEST['action']) {
            case "delete":
                if(!in_array($ID,$arrNotDel)) {
                    @set_time_limit(0);
                    $DB->StartTransaction();
                    if (!CTenderixSprDetails::Delete($ID)) {
                        $DB->Rollback();
            ...
            case "deactivate":
                ...
                if (!$DB->Update("b_tx_spr_details", $arFields, "WHERE ID='$ID'", $err_mess . __LINE__))
...
$rsData = CTenderixSprDetails::GetList($by, $order, $arFilter, $is_filtered);
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if ($TENDERIXRIGHT == "W") {
        ...
        $row->AddViewField("TITLE", '<a href="tenderix_spr_details_edit.php?lang=' . LANGUAGE_ID . '&ID=' . $f_ID . '&find_spr_id=' . $find_spr_id . '" title="'.GetMessage("PW_TD_EDIT_TITLE").'">' . $f_TITLE . '</a>');
    } else {
    ...
    $arActions = Array();
    $arActions[] = array("DEFAULT" => "Y", "ICON" => "edit", "TEXT" => GetMessage("MAIN_ADMIN_MENU_EDIT"), "ACTION" => $lAdmin->ActionRedirect("tenderix_spr_details_edit.php?ID=" . $f_ID . '&find_spr_id=' . $find_spr_id));
    if ($f_ID != '1' && $TENDERIXRIGHT == "W") {
        $arActions[] = array("SEPARATOR" => true);
        $arActions[] = array("ICON" => "delete", "TEXT" => GetMessage("MAIN_ADMIN_MENU_DELETE"), "ACTION" => "if(confirm('" . GetMessage("PW_TD_CONFIRM_DEL_CHANNEL") . "')) window.location='tenderix_spr_details.php?lang=" . LANGUAGE_ID . '&find_spr_id=' . $find_spr_id . "&action=delete&ID=$f_ID&" . bitrix_sessid_get() . "'");
    }
...
$aMenu[] = array(
    "TEXT" => GetMessage("SPR_LIST"),
    "TITLE" => GetMessage("SPR_TITLE_LIST"),
    "LINK" => "/bitrix/admin/tenderix_spr.php?lang=" . LANGUAGE_ID,
    "ICON" => "btn_list");
...
if ($TENDERIXRIGHT == "W") {
    $aMenu[] = array(
        ...
        "LINK" => "tenderix_spr_details_edit.php?lang=" . LANG . '&find_spr_id=' . $find_spr_id,
        "ICON" => "btn_new"
    );
}
...
/* * ******************************************************************
  Form
 * ****************************************************************** */
$APPLICATION->SetTitle(GetMessage("PW_TD_PAGE_TITLE") . $arSprTitle);
