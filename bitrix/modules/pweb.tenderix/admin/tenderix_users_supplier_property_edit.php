...
if ((strlen($save) > 0 || strlen($apply) > 0) && $REQUEST_METHOD == "POST" && $TENDERIXRIGHT == "W" && check_bitrix_sessid()) {
    ...
    $res = 0;
    if ($ID > 0)
        $res = CTenderixUserSupplierProperty::Update($ID, $arFields);
    else
        $res = CTenderixUserSupplierProperty::Add($arFields);
    if (intVal($res) <= 0 && $e = $GLOBALS["APPLICATION"]->GetException()) {
        ...
        LocalRedirect("tenderix_users_supplier_property.php?lang=" . LANG . "&" . GetFilterParams("filter_", false));
    else
        $ID = $res;
}

if ($ID > 0) {
    $db_enterprise = CTenderixUserSupplierProperty::GetList($by, $order, array("ID" => $ID));
    $db_enterprise->ExtractFields("str_", False);
}

if ($bInitVars) {
    $DB->InitTableVarsForEdit("b_tx_supplier_property", "", "str_");
}
...
$aMenu = array(
    array(
        ...
        "LINK" => "/bitrix/admin/tenderix_users_supplier_property.php?lang=" . LANG . "&" . GetFilterParams("filter_", false),
        "ICON" => "btn_list",
    )
);
...
if ($ID > 0 && $TENDERIXRIGHT == "W") {
    ...
    $aMenu[] = array(
        ...
        "LINK" => "/bitrix/admin/tenderix_users_supplier_property_edit.php?lang=" . LANG . "&" . GetFilterParams("filter_", false),
        "ICON" => "btn_new",
    );
    ...
    if ($ID != '1') {
        $aMenu[] = array(
            ...
            "LINK" => "javascript:if(confirm('" . GetMessage("PW_TD_DELETE_ENT_CONFIRM") . "')) window.location='/bitrix/admin/tenderix_users_supplier_property.php?action=delete&ID[]=" . $ID . "&lang=" . LANG . "&" . bitrix_sessid_get() . "#tb';",
            "ICON" => "btn_delete",
        );
    }
}
...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>" name="property_edit" enctype="multipart/form-data">
    ...
    $tabControl->Buttons(
            array(
                ...
                "back_url" => "/bitrix/admin/tenderix_users_supplier_property.php?lang=" . LANG . "&" . GetFilterParams("filter_", false)
            )
    );
    $tabControl->End();
    ?>
