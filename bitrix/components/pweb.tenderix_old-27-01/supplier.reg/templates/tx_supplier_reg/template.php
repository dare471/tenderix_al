...
Более широкий перечень документов может быть запрошен в рамках каждой отдельной конкурсной процедуры.
		...
		<? endif; ?>
		<input type="checkbox" id="kontur_agree" name="INFO[AGREE]" value="" <? echo ($arResult["INFO"]["AGREE"] == "Y" ? "checked" : "") ?> /> Я согласен(-на) на обработку моих персональных данных. <a href="https://152.kontur.ru/725375589/policy" tareget="_blank">Политика конфиденциальности</a> <br/>
		<script>
		...
		<? if(isset($arResult["INFO"]["STATUS_ID"]) && ($arResult["INFO"]["STATUS_ID"] == 6)) { ?>
			<button style="color:#fff!important;" class="btn btn-primary" name="reg_submit_yet"><?=GetMessage("PW_TD_FORM_REG_YET"); ?></button>
		<? } else {?>
			<input type="submit" style="color:#fff!important;" class="btn btn-primary" name="reg_submit" <? echo ($arResult["INFO"]["AGREE"] == "Y" ? "" : "disabled") ?> value="<?= (($arResult["INFO"]["STATUS_ID"] == 7) ? GetMessage("PW_TD_FORM_SAVE") : GetMessage("PW_TD_FORM_REG")); ?>" />
		<? } ?>
	</form>
</div>
							<script>
