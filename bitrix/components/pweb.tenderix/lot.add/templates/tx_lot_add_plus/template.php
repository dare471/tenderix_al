...
<?if(($arResult["LOT"]["ACTIVE"] == "Y") && ($arResult["LOT"]["NOEDIT"] == "Y") && ($arResult["COPY"] == "N") && ($date_tek > $date_end) && ($date_end != 0)) :?>
	...
	<?$dis_arch = " disabled";?>
<?elseif($arResult["T_RIGHT"] != "W" && (($arResult["LOT"]["ACTIVE"] == "Y") && ($arResult["COPY"] == "N") && ($date_tek < $date_end) && ($date_tek > $date_start) && ($date_end != 0) && ($date_start != 0))): ?>
	<br />
	<br />
	<div class="well">
			...
			$.ajax({
				...
				success: function (data) {
					for (var i = 0; i < data.length; i++) {
						$("#select-all-supplier").append('<option value="' + data[i].id + '">' + data[i].company2 + '</option>');
					}
		...
		if ($('#searchSupplier').val().length >= 0) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					$('#select-all-supplier').empty();
					for (var i = 0; i < data.length; i++) {
						$("#select-all-supplier").append('<option value="' + data[i].id + '" selected>' + data[i].company2 + '</option>');
					}
				}
			});
...
function arrItem() {
	...
	$.ajax({
		url: "<?= $templateFolder ?>/ajax.php",
		type: "POST",
		data: "type=<?php echo $arResult['TYPE_ID'];?>&action=addItem&numProp=" + numProp + "&newProp=" + newProp,
		beforeSend: function () {
...
<div class="lot-add">
	...
	<form name="lotadd_form" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data">
		<!--h3><?= GetMessage("PW_TD_BASE_TITLE") ?></h3-->
				<?if(!$arResult['LOT']['ID']):?>
					<?if($_GET['TYPE_ID'] == 'T'):?>
						<div class="btn btn-gray">Конкурс</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<div style="background:#adadad;" class="btn " ><a style="color:#fff!important;" href="lot.php?TYPE_ID=N">Аукцион</a></div>
					<?else:?>
						<div style="background:#adadad;" class="btn "><a style="color:#fff!important;" href="lot.php?TYPE_ID=T">Конкурс</a></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<div class="btn btn-gray">Аукцион</div>
					<?endif;?>
					<br>
				<?endif;?>
		<br/>
		...
		<table class="table table-striped table-bordered table-hover table-condensed">
			<? if ($arResult["LOT"]["ID"] > 0 && $arResult["COPY"] == "N"): ?>
				<tr>
					...
					<td width="60%" class="right-col"><? echo $arResult["LOT"]["ID"] ?>
						<?if($arResult['LOT']['LINKED'] > 0):?> [<a href="/tenders_detail.php?LOT_ID=<?=$arResult['LOT']['LINKED']?>">Второй этап</a>]<?endif;?>
						<?if($arResult['TYPE_ID'] == 'T'):?>
							<span style="float:right;" class="label label-info">Конкурс</span>
						<?endif;?>
						<?if($arResult['TYPE_ID'] == 'N'):?>
							<span style="float:right;" class="label label-primary">Аукцион</span>
						<?endif;?>
					</td>
				</tr>
			<? endif; ?>
			...
			<? endif; ?>
			<? if ($arResult["LOT"]["ARCHIVE"] == "Y" && $arResult["LOT"]["ID"] > 0 && $arResult["COPY"] == "N"): ?>
				<tr>
					<td width="40%" class="left-col">
						Лот в архиве:
					</td>
					<td width="60%" class="right-col">
						<input type="checkbox" name="ARCHIVE" value="Y" <?if ($arResult["LOT"]["ARCHIVE"] == "Y") echo " checked"; ?> <?= $dis_arch; ?> />
					</td>
				</tr>
			<? endif; ?>
			<? if ($arResult["LOT"]["FAIL"] == "Y" && $arResult["LOT"]["ID"] > 0 && $arResult["COPY"] == "N"): ?>
				<tr>
					<td width="40%" class="left-col">
						Лот не состоялся:
					</td>
					<td width="60%" class="right-col">
						<input type="checkbox" name="FAIL" value="Y" <?if ($arResult["LOT"]["FAIL"] == "Y") echo " checked"; ?> <?= $dis_arch; ?> />
					</td>
				</tr>
			<? endif; ?>
<!-- 	                ТИП ЛОТА -->
			<? if ($arResult["LOT"]["ID"] <= 0 && $arResult["COPY"] == "N"): ?>
				<? if($arResult["TYPE_ID"] == "N" || $arResult["TYPE_ID"] == "P") { ?>
					<tr>
						<td width="40%" class="left-col">
						...
						<td width="60%" class="right-col">
							<div class="form-group">
								<select class="form-control input-sm"
									...
									<option<?= $arResult["TYPE_ID"] == "N" ? " selected" : ""; ?> value="<?= $APPLICATION->GetCurPageParam("TYPE_ID=N", array("TYPE_ID", "SECTION_ID", "PRODUCTS_ID")) ?>"><?= GetMessage("PW_TD_TYPE_PRODUCT_ST") ?></option>
			...
			<tr>
				...
				<td width="60%" class="right-col">
					...
					<?else:?>
						...
						<?endif;?>
								<? foreach ($arResult["SECTION_ARR"][0] as $sec): ?>
								<option<?= $_REQUEST["SECTION_ID"] == $sec["ID"] ? " selected" : ""; ?> value="<?= $sec["ID"] ?>"><?= $sec["TITLE"] ?></option>
								<? endforeach; ?>
							<? foreach ($arResult["CATALOG"] as $cat_id => $cat_name) : ?>
								<optgroup label="<?= $cat_name ?>">
									<? foreach ($arResult["SECTION_ARR"][$cat_id] as $sec): ?>
...
<!-- НАИМЕНОВАНИЕ ТОВАРА СПЕЦИФИКАЦИЯ -->
			<?if (($arResult["TYPE_ID"] != 'S') && ($arResult["TYPE_ID"] != 'R')): ?>
				...
				<? endif;*/ ?>
				<? if ($arResult["TYPE_ID"] == 'N' || $arResult["TYPE_ID"] == 'T' || $arResult["TYPE_ID"] == 'P'): ?>
					<tr>
						<td width="40%" class="left-col">
							<?= GetMessage("PW_TD_FULL_SPEC") ?>:
						</td>
						<td width="60%" class="right-col">
							<input type="checkbox" name="FULL_SPEC" value="Y"<?= $arResult["LOT"]["FULL_SPEC"] == "N" ? "" : " checked"; ?> <?= $dis_arch; ?> />
						</td>
					</tr>
					<tr>
						<td width="40%" class="left-col">
			...
			<tr>
				...
				<td width="60%" class="right-col">
					<input type="checkbox" name="OPEN_PRICE" id="OPEN_PRICE" value="Y"<?= $arResult["LOT"]["OPEN_PRICE"] == "Y" ? " checked" : ""; ?> <?= $dis_arch; ?> />
				</td>
			</tr>
			<script>
				$("#OPEN_PRICE").click(function () {
					if($(this).prop('checked')) {
						$("#NOSAME").removeAttr('disabled');
					}
					else {
						$("#NOSAME").attr('disabled','disabled');
						$("#NOSAME").removeAttr('checked');
					}
				});
			</script>
			...
			<tr>
				<td width="40%" class="left-col">
					Запрещать поставщикам устанавливать равные ставки:<br />
				</td>
				<td width="60%" class="right-col">
					<input type="checkbox" name="NOSAME" id="NOSAME" title="работает при установленном признаке лота с открытой ценой (выше)" value="Y"<?= $arResult["LOT"]["NOSAME"] == "Y" ? " checked" : ""; ?> <?= ((!isset($arResult["LOT"]["OPEN_PRICE"]) || ($arResult["LOT"]["OPEN_PRICE"] == "N")) ? " disabled" : ""); ?> />
				</td>
			</tr>
			...
			<!--tr>
				<td width="40%" class="left-col">
					Рассылка со спецификацией по окончанию конкурса:
				</td>
				<td width="60%" class="right-col">
			...
			<!--tr>
				...
				<td width="60%" class="right-col">
					<input type="checkbox" name="VIZ_HIST" value="N"<?//= $arResult["LOT"]["VIZ_HIST"] == "Y" ? " checked" : ""; ?> <?//= $dis_arch; ?> />
				</td>
			</tr-->
			
			<tr>
				<td width="40%" class="left-col">
					Возможность делать ставки до открытия лота:
				</td>
				<td width="60%" class="right-col">
			...
			<? //if ($arResult["TYPE_ID"] != 'P'):?>
				...
				<tr>
					<td width="40%" class="left-col">
						<!--span class="required">*</span--><?= GetMessage("PW_TD_PRIVATE") ?>:
					</td>
					<td width="60%" class="right-col">
										...
										<td>
											<select id="select-all-supplier" class="supplier-select" multiple size="20">
												<? if ($arResult["LOT"]["PRIVATE"] == "Y"): ?>
													...
													<option value="<?= $arSupplier["id"] ?>"><?= $arSupplier["company"] ?> [<?= $arSupplier["email"] ?>]</option>
													<? endforeach; ?>
												<? endif; ?>
											</select>
										</td>
											...
											<select name="PRIVATE_LIST[]" id="select-private-supplier" class="supplier-select" multiple size="20">
												...
												<?// TODO?>
													<? foreach ($arResult["LOT"]["PRIVATE_LIST"] as $arSupplier) : ?>
														<option selected value="<?= $arSupplier["id"] ?>"><?= $arSupplier["company"] ?> [<?= $arSupplier["email"] ?>]</option>
													<? endforeach; ?>
												<? endif; ?>
											</select>
										</td>
			...
			<?else:?>
			<input class="form-control input-sm hidden" type="text" name="TIME_EXTENSION" value="0" size="10" <?= $dis_arch; ?> />
			<input class="form-control input-sm hidden" type="text" name="TIME_UPDATE" value="600" size="10" <?= $dis_arch; ?> />
			<?endif;?>
			...
			<tr>
				...
				<td width="60%" class="right-col">
					<? if ($arParams["VISUAL_EDITOR"] == "Y"): ?>
						...
						?>
					<? else: ?>
					<textarea class="form-control" name="NOTE" cols="50" rows="15" <?= $dis_arch; ?>><?= htmlspecialcharsEx($arResult["LOT"]["NOTE"]) ?></textarea>
					<? endif; ?>
				</td>
			</tr>
			<tr class="heading">
			...
			<tr>
				<td class="reg-field" width="40%" class="left-col">
					<ol>
						<li>Обязательно приложите разработанный шаблон спецификации в Excel для заполнения поставщиком.</li>
						<li>Обязательно приложите список документов, предоставляемых поставщиком в рамках лота, а также их формат.</li>
						<li>Обязательно приложите список документов, предоставляемых поставщиком в случае победы.</li>
						<li>Обязательно приложите техническое задание.</li>
					</ol>
				</td>
				<td width="60%" class="right-col">
					<div class="form-group">
			...
			<tr class="heading">
				<td colspan="2">Рассылка уведомлений о лоте</td>
			</tr>
			...
			<tr>
				...
				<td>
					<div class="form-group">
						<input class="form-control input-sm" type="text" name="SUBSCR_END" value="<?if($arResult['LOT']['SUBSCR_END']) {echo intval($arResult["LOT"]["SUBSCR_END"]);} else { echo '1440';}?>" size="10" <?= $dis_arch; ?> />
					</div>
				</td>
			</tr>
		</table>
		...
		<?//__($arResult);?>
		<?	$k = 0;
			foreach($arResult['DOPPROPLOT'] as $dopProp):
				//__($dopProp);
				if($dopProp['ACTIVE'] == 'Y' && $dopProp['START_LOT'] == 'Y' && $dopProp['T_RIGHT'] == 'W'):?>
					<?//__($dopProp);?>
					<p><?=$dopProp['DESCRIPTION']?></p>
					<input class="form-control input-sm" type="hidden" name="DOPPROP[<?=$k?>][ID]" value="<?=$dopProp["ID"] ?>" style="width: 30%" <?= $dis_arch; ?> />
					<input class="form-control input-sm" type="text" placeholder="<?=$dopProp['TITLE']?>" name="DOPPROP[<?=$k?>][VALUE]" value="<?if($arResult['DOPPROP'][$dopProp["ID"]]): echo $arResult['DOPPROP'][$dopProp["ID"]]; endif;?>" style="width: 30%" <?= $dis_arch; ?> /><br />
					<?
					$k++;
				endif;
			endforeach;
		...
		<?if ($arResult["TYPE_ID"] != 'S' && $arResult['TYPE_ID'] != 'R') : ?>
			...
			<li>Заполните спецификацию СЭТ (итоговые строки) в соответствии с шаблоном Excel.</li>
			<table id="table_spec" cellpadding="0" cellspacing="0" width="100%" class="t_lot_table table table-striped table-hover table-condensed">
				<tr>
					...
					<td align="center" width="30%"><? echo GetMessage("PW_TD_SPEC_NAME") ?></td>
					<td align="center" width="38%"><? echo GetMessage("PW_TD_SPEC_DOP") ?></td>
					<td align="center" width="10%"><? echo GetMessage("PW_TD_SPEC_UNIT") ?></td>
					<td align="center" width="7%"><? echo GetMessage("PW_TD_SPEC_COUNT") ?></td>
					<td align="center" width="15%"><? echo GetMessage("PW_TD_SPEC_START_PRICE") ?></td>
					<?if ($arResult["TYPE_ID"] == 'N'):?><td align="center" width="10%"><? echo GetMessage("PW_TD_SPEC_STEP_PRICE") ?></td><?endif;?>
					<?if ($arResult["TYPE_ID"] == 'P'):?><td align="center" width="10%"><? echo GetMessage("PW_TD_SPEC_STEP_PRICE_UP") ?></td><?endif;?>
				...
				<? foreach ($arResult["LOT"]["SPEC"] as $kkey => $PROP): ?>
					<? //if (is_numeric($kkey)) :?>
						<tr>
							<td align="center" <?if($PROP["COUNT"] == '0') {echo 'style="color:red;"';} ?>><? echo $numProp ?></td>
							<td align="center">
								<div class="form-group">
									<input <?if($PROP["COUNT"] == '0') {echo 'style="color:red;"';} ?> class="form-control input-sm" type="text" name="PROP_<?= $PROP["PROP_ID"] ?>_TITLE" value="<?= htmlspecialcharsEx($PROP["TITLE"]) ?>" style="width: 98%" <?= $dis_arch; ?> />
								</div>
							</td>
							<td align="center">
								<div class="form-group">
									<input <?if($PROP["COUNT"] == '0') {echo 'style="color:red;"';} ?> class="form-control input-sm" type="text" name="PROP_<?= $PROP["PROP_ID"] ?>_ADD_INFO" value="<?= htmlspecialcharsEx($PROP["ADD_INFO"]) ?>" style="width: 98%" <?= $dis_arch; ?> />
								</div>
							</td>
							<td align="center">
								<div class="form-group">
							...
							<td align="center">
								<div class="form-group">
									<input <?if($PROP["COUNT"] == '0') {echo 'style="color:red;"';} ?> class="form-control input-sm" type="text" name="PROP_<?= $PROP["PROP_ID"] ?>_COUNT" value="<?= $PROP["COUNT"] ?>" <?= $dis_arch; ?> />
								</div>
							</td>
		...
		<? if ($arResult["TYPE_ID"] != 'S' && $arResult["TYPE_ID"] != 'R'): ?>
			<div class="container">
				<div class="row">
					<input class="btn btn-gray btn-sm" type="button" id="addItem" value="<?= GetMessage("PW_TD_BUTTON_ADD_ITEM") ?>"<?= $dis_arch; ?> />       
				</div>
			</div>
		<? endif; ?>
		<br/>

		<?if(count($arResult['LOT']['SPEC']) <= 0):?>
		<div class="alert alert-danger">Вы не заполнили спецификацию СЭТ. Заполните и сохраните или примените настройки лота! </div>
		<?endif;?>

		<?if($arResult['LOT']['PRIVATE'] == 'Y' && count($arResult['LOT']['PRIVATE_LIST']) <= 0):?>
		<div class="alert alert-danger">Вы сделали лот закрытым, но не выбрали поставщиков. Добавьте постащиков и сохраните или примените настройки лота!</div>
		<?endif;?>
		<div class="container">
			<div class="row">
				<? if(!isset($arResult["LOT"]["ID"]) || $arResult["LOT"]["ACTIVE"] == "N") { ?>
				<input class="btn btn-add-submit" type="submit" name="lotadd_submit" value="<?= GetMessage("PW_TD_ADD_LOT") ?>" <?= $dis_arch; ?> />&nbsp;
				<? } ?>
				<input class="btn btn-add-submit" type="submit" name="lotapply_submit" value="Применить" <?= $dis_arch; ?> />&nbsp;
				<? if ($arResult["T_RIGHT"] == "W"): ?>
					<?if(($arResult['LOT']['PRIVATE'] == 'Y' && count($arResult['LOT']['PRIVATE_LIST']) <= 0) || count($arResult['LOT']['SPEC']) <= 0):?>
					<?else:?>
					<input class="btn btn-add-submit" type="submit" name="lotopen_submit" value="<?= GetMessage("PW_TD_OPEN_LOT") ?>" <?= $dis_arch; ?> />&nbsp;
					<?endif;?>
