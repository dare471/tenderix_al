		...
		<h2>
			...
			<? 	if($arResult["LOT"]["ACTIVE"] == "Y") { ?>
					<?if ($arResult["LOT"]["PRIVATE"] == "Y"):?>
						<i class="t_lot_status t_lot_status_private fa fa-users" title="Закрытый лот" data-toggle="tooltip" data-placement="left"></i>&nbsp;
					<?endif;?>
					...
					<?}elseif ($arResult["LOT"]["END_LOT"] == "Y") {?>
						<i class="t_lot_status t_lot_status_end fa fa-lock" title="Завершенный лот" data-toggle="tooltip" data-placement="left"></i>&nbsp;
					<?}else{?>
...
<div class="side-tab">
	<div class="side-tab_menu">
		<ul class="side-tab_tabs">
			...
			<li class="side-tab_tab" onclick="changeTab('bet', this)" id="secondTab"><span class="label">Мои ставки</span></li>
	...
	<div class="side-tab_content" id="basic" style="display:block">
		<div class="col-md-12">
		<form name="proposal_add"  id="proposal_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data">
		...
		<div class="row mb-3">
					...
					<table class="t_lot_table">
						...
						<tr>
							...
							<? if ($arResult["LOT"]["TYPE_ID"] == "R" || $arResult["LOT"]["TYPE_ID"] == "T"): ?>
								<b><span class="label label-info">КОНКУРС<?if($arResult['LOT']['QUOTES'] == 'Y'):?>(Запрос котировок)<?endif;?></span></b>
							<?else:?>
								<? if ($arResult["LOT"]["TYPE_ID"] == "P"): ?>
									<b>Продажа (повышение цены)</b>
									<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;Установка ставок ниже начальной запрещена.</b>
								<?else:?>
									<b>Покупка (понижение цены)</b>
									<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;Установка ставок выше начальной запрещена.</b>
								<?endif;?>

								<? if ($arResult["LOT"]["NOSAME"] == "Y"): ?>
									<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;Установка равных ставок в лоте запрещена.</b>
								<? endif; ?>

								<? if ($arResult["LOT"]["NOBAD"] == "Y"): ?>
									<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;Установка ставок хуже лидирующей запрещена.</b>
								<? endif; ?>

								<? if ($arResult["LOT"]["ONLY_BEST"] == "Y"): ?>
									<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;Установка ставок хуже начальной запрещена.</b>
								<? endif; ?>

								<? if ($arResult["LOT"]["PRE_PROPOSAL"] == "Y"): ?>
									<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;Возможность делать ставки до начала торгов.</b>
								<? endif; ?>
							<?endif;?>
							<? if ($arResult["LOT"]["OPEN_PRICE"] == "Y"): ?>
								<br/><b><i class="fa fa-exclamation-triangle t_lot_exclamation"></i>&nbsp;&nbsp;<?= GetMessage("PW_TD_LOT_OPEN_PRICE") ?></b>
							<? endif; ?>
							</td>
						</tr>
						<tr>
							<td><b><?= GetMessage("PW_TD_CURR") ?></b></td>
							<td>
								<select name="CURRENCY_PROPOSAL" class="currency-proposal">
							...
							<td>
								...
								foreach ($arResult["PROPERTY_SPEC"] as $specProp) {
									...
									if (isset($arResult["PROPOSAL_SPEC"]) && $arResult["PROPOSAL_ID"] > 0) {
										$sumProposal += $arResult["PROPOSAL_SPEC"][$specProp["ID"]]["PRICE_NDS"]*$specProp['COUNT'];
									}
								}
								?>
								<span id="total_start_price"><b><?=number_format($sumStart, 2, '.', ' ');?></b></span>
								<input type="hidden" id="total_start_price_full" value="<?=number_format($sumStart, 10, '.', ' ');?>" />
							</td>
						</tr>
						<tr>
							...
							<td>
								<? 
								if ( $sumProposal == $arResult['MIN_PRICE'] && $sumProposal != 0) {
									
									?> <span id="total_min_price" style="color:green"><b><? echo number_format($arResult['MIN_PRICE'], 2, '.', ' ');?></b></span><br/>
								...
								<? } else { ?>
									...
									<? if ($arResult['MIN_PRICE'] == 0): ?>
										-
									<? else:?>
										<?=number_format($arResult['MIN_PRICE'], 2, '.', ' ');?>								
									<?endif;?>
									</b>
									</span>
									<input type="hidden" id="total_min_price_full" value="<? echo number_format($arResult['MIN_PRICE'], 10, '.', ' ');?>" />
			...
			<div class="col-md-5">
				<div class="t_block" style="position:relative;">
					<div class="t_block_header">Уведомления</div>
					<div class="t_block_content">
						
						<? if ($arResult["TIME"] == "S") { 
							echo "<div class='alert alert-danger t_lot_pre_status'>";
							ShowError($arResult["TIME_S_MESSAGE"]);
							echo "</div>";
						}?>
					
						<? if ($arResult["TIME"] == ""): ?>
							<div class="alert alert-info" style="font-size:18px;text-align: center;">
								<b><?= GetMessage("PW_TD_BALANCE_TIME") ?>:</b> </br><span id="time"></span> <span id="time2"></span>
							</div>							
							<?if (($bestpr_usid != $USER->GetID()) && ($bestpr_usid != 0) && $arResult['LOT']['OPEN_PRICE'] == 'Y'): ?> 
								<div class="alert alert-warning" style="font-size:18px;text-align: center;">
									<b>Лучшее предложение сделано другим поставщиком.</b>
								</div>
							<? endif; ?>							
						<? endif; ?>
							...
							<div class="row">
								<div class="col-md-12">
									...
									<? if ($arResult["SEND_OK"] == "Y"): ?>
										<div class="alert alert-success alert-dismissible show fade" style="text-align: center">
											<button class="close" type="button" data-dismiss="alert">×</button>
											<?= GetMessage("PW_TD_SEND_OK") ?>
										</div>
									<? endif; ?>
								</div>
							</div>
						<? endif; ?>
						<?if ($arResult["TIME"] == ""): ?>
						<div class="t_block_footer" style="text-align: center;">
							<button class="btn btn-refresh" onclick="location.reload();return false;"><i class="fa fa-sync-alt btn-refresh-icon"></i>&nbsp;Обновить</button>
							<div class="mb-2"></div>
						</div>
						<?endif;?>
					</div>
		...
		<?if ($arResult["TIME"] == ""): ?>
			...
			<div style="margin-top:20px;position:relative;" class="alert alert-warning message-dopprop">
				<h4>Спецификация СЭТ заполняется только в тенге.</h4>
				...
				<div class="collapse in" id="collapseExample">
					<p>В случае подачи предложения в иной валюте - для заполнения Спецификации СЭТ, необходимо сконвертировать все цены в тенге по курсу, указанному в Лоте. <br>
			...
			<? foreach ($arResult["PROPERTY_SPEC"] as $specProp): ?>
				...
				<? if ($numProp == 1): ?>
					<tr>
						...
						<? endif; ?>
						<? if ($arResult["LOT"]["OPEN_PRICE"] == "Y" && $arResult["TIME"] == ""): ?>
				...
				<tr<?= $gr_style; ?>>
					...
					<td class="item-proposal-price">
						...
						?><div style="display:none;"><?=$priceNDS;?></div><?
					...
					<? if ($arResult["LOT"]["OPEN_PRICE"] == "Y" && $arResult["TIME"] == ""): ?>
						<td align="center" class="best_proposal_n" id="best_proposal_<?= $specProp["ID"] ?>">
						
						<?if(isset($specProp["ID"])):?><strong>Предложений еще нет</strong><?endif;?>
						</td>
	...
	<? if ($arResult["TIME"] == ""):?>
		...
		<div class="t_right">
			...
			<?  /*if(!empty($arResult["LOT"]["ACCESS"])) {
					if($arResult["LOT"]["ACCESS"] == "Y") {*/ ?>
						<? if ($arResult["LOT"]["TYPE_ID"] == "R" || $arResult["LOT"]["TYPE_ID"] == "T"): ?>
							<input class="btn" type="submit" name="proposal_submit" value="Сделать предложение"/><br/><br/>
						<?else:?>
							<input class="btn" type="submit" name="proposal_submit" value="<?= GetMessage("PROPOSAL_FORM_SAVE") ?>"/><br/><br/>
						<?endif;?>
							<input type="hidden" name="proposal_submit" value="Y" />
					<? /*} else { ?>
							<input class="btn btn-primary proposal_request" type="button" name="proposal_request_yet" value="<?= GetMessage("PW_TD_REQUEST_YET") ?>" disabled /><br/><br/>
					<? }
					} else {?>
...
<? if ($arResult["TIME"] == ""): ?>
	...
	function stepUp(id, step, num, price) {
		...
		var curr_base = $("input[name='CURRENCY_PROPOSAL']").val();
		var curr_user = $("[name='CURR_USER']").val();
		var curr_val = parseFloat($("[name='CURR[" + curr_base + "]']").val());
		var curr_val2 = parseFloat($("[name='CURR[" + curr_user + "]']").val());

		var curr_val = curr_val2/curr_val;
		...
		price_nds = price_nds.replace(/\s/g, '');
		...
		<? if($arResult["LOT"]["TYPE_ID"] == "N") { ?>
			...
			//else {
				...
				$("input[name='PROP_" + id + "_PRICE_NDS']").val(formatNumber(price_nds.toFixed(2)));
				$("input[name='PROP_" + id + "_PRICE_NDS_dis']").val(formatNumber(price_nds.toFixed(2)));
			//}
	...
	function stepDown(id, step, num, price) {
		...
		if (price_nds <=0) {
			$("input[name='PROP_" + id + "_PRICE_NDS_dis']").css("border", "1px solid red");
			$("#PROP_"+ id +"_err_span").html("нельзя устанавливать<br /> отрицательную цену");
			return false;
		}
					...
					if (json.time_diff <= 0) {
						...
						if (json.type != "S") {
							...
							for (var key in proposal_min) {
								$("#best_proposal_" + key).html(formatNumber(proposal_min[key].toFixed(2)));
								$("#best_proposal_full_" + key).val(proposal_min[key]);
								$("#best_proposal2_" + key).html(formatNumber(proposal_min[key].toFixed(2)));
...
<? endif; ?>
	...
	<div class="side-tab_content" id="bet">
		<div class="col-md-12">
			<div class="t_block_header">Мои ставки</div>
			<?
					...
					foreach ($lspec as $bid => $pspec) {
						...
						if ($curpr[$ff][$bid] != $curpr[$ff - 1][$bid]) {
							...
							<tr>
								...
								<td align="center"><?= number_format(($pspec["PRICE_NDS"]), 2, '.', ' '); ?></td>
...
<div id="popup-confirm-proposal" class="t_popup-window-content">
	<div class="t_popup-content-header">
		<h4>Вы действительно хотите сделать ставку?</h4></br>
	</div>
	<div id="popup-confirm-prop-body" class="t_popup-content-body">
...
<script>
	BX.ready(function(){
	   var confirmProposalSubmit = new BX.PopupWindow("schema", null, {
			content: BX('popup-confirm-proposal'),
			className: 't_popup-window-layout',
			closeIcon: {right: "20px", top: "10px", className: "t_icon"},
			titleBar: {
				content: BX.create("div", {html: '<b>Подача предложения</b>', 'props': {'className': 't_popup-title-bar'}}),
				className: 't_popup-window-title',
			},
			zIndex: 0,
			offsetLeft: 0,
			offsetTop: 0,
			...
			$('input[name="proposal_submit"]').click(function(e){;
				var content;
				var arrProps = $('input[name$="PRICE_NDS"]');
				
				// Определяем заголовок столбца в зависимости от типа НДС и валюты
				var withNds = '<?= $arResult["LOT"]["WITH_NDS"] ?>';
				var currency = '<?= $arResult["LOT"]["CURRENCY"] ?>';
				var priceHeader;
				
				if (currency == 'KZT' && withNds == 'Y') {
					priceHeader = 'Цена за ед. с НДС';
				} else {
					priceHeader = 'Цена за ед. без НДС';
				}
				
				content = '<table class="t_popup-window-table">';
				content += 	'<tr>'+
								'<th>#</th>'+
								'<th>Наименование товара</th>'+
								'<th>'+priceHeader+'</th>'+
							'</tr>';
				arrProps.each( function(index) {
				...
				$('#popup-confirm-prop-body').html(content);
				
				confirmProposalSubmit.show();	
				e.preventDefault();
			});
	});
</script>
